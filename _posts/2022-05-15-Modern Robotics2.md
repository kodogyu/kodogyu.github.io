...
한개 이상의 closed-loop를 갖는 일반적인 로봇의 C-space는 implicit하게 열벡터 θ = [θ<sub>1</sub>, θ<sub>2</sub>, ···, θ<sub>4</sub>]<sup>T</sup> 로 표현됩니다.  
...
(식 2.5)는 로봇의 C-space를 제한하는 constraint(위치에 대한 제약)라서 C-space의 차원을 낮춥니다.  
